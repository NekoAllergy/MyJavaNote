...
html > (jasper)를 통해 > jsp파일을 java파일로 바꿔준다.
직접 html을 출력문에 넣지 않아도 된다..ㅠㅠㅠㅠㅠ
...
자바에서 
조건문을 통해 결과값을 출력할 경우(if 조회수 200이상 게시글)
DB의 모든 데이터를 가져와 그 중에 해당되는 조건만 출력함
>> 비효율적

자바에서 
SELECT * FROM MEMBER WHERE ID = 10 이라는 쿼리문을 보낼 경우
DB에서 해당되는 조건만 가져와 출력함
>> 효율적임
